...
import numpy as np
from data.data_processing.data_processing import read_image, read_label, normalize, resize_images

from data.data_processing.data_processing import retrieve_mask_mappings, map_labels_tf, augment_image_and_label
...
# def test_load_dataset_unet():
#     # Mock get_datasets to return dummy paths
#     with patch('data_processing.get_datasets') as mock_get_datasets:
...
#         dummy_image = np.random.randint(0, 255, (1024, 2048, 3), dtype=np.uint8)
#         dummy_label = np.random.randint(0, 8, (1024, 2048, 1), dtype=np.uint8)
